...
Each document contains a `title`, `uid`, and text content — possibly including `[Image Caption: ...]`, `[Image Path: ...]`, \
or handwritten content marked with `[HandWriting Content Start] ... [HandWriting Content End]`. \
Treat content inside the handwriting tags as handwritten content, but only if there is meaningful (non-empty, only newlines, non-whitespace) text between the tags. \
If the tags are empty or contain no usable content, ignore them and treat the document as not containing handwriting. \
Use this context to answer the question. If handwriting or an image is relevant, refer to it directly. \
